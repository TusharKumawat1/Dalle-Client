import React from 'react'
import { download } from '../assets'
import { downloadImg } from '../utils'
export default function Card( {_id,name ,prompt,photo}) {
  return (
    <div className='rounded-xl group relative shadow-card hover:shadow-cardhover card'>
    <img className='w-full h-auto object-cover rounded-xl'
    src={photo}
    alt={prompt}>
    </img>
    <div className='group-hover:flex flex-col max-h-[94.5%] hidden absolute bottom-0 left-0 right-0 bg-[#10131f] m-2 p-4 rounded-md'>
      <p className='text-white text-sm overflow-auto '>{prompt}</p>
      <div className='mt-5 flex justify-between item-center gap 2'>
      <div className='flex item-center gap-2 '>
        <div className='w-6 h-6 rounded-full bg-green-700 text-white flex justify-center items-center font-bold'>
          {name[0]}
        </div>
        <p className='text-white text-sm'>{name}</p>
      </div>
      <button onClick={()=>{downloadImg(_id,photo)}} className='outline-none bg-transparent '>
        <img src={download} alt="download" className='object-contain w-6 h-6 invert'/>
      </button>
      </div>
    </div>
    </div>
  )
}
